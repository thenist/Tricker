...
def get_diet(code, ymd, weekday, website, schoolcode, level):
    ...
    if weekday == 5 or weekday == 6: #토요일,일요일 버림
        ...
        URL = (
                "%ssts_sci_md01_001.do?"
                "schulCode=%s"
                "&schulCrseScCode=%d"
                "&schulKndScCode=0%d"
                "&schMmealScCode=%d&schYmd=%s" % (website, schoolcode,level, level, schMmealScCode, schYmd)
            )
